...
export const UserInfoForm: React.FC<UserInfoFormProps> = ({ onSubmit }) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.phone.trim()) newErrors.phone = 'Date of Birth is required';
    setErrors(newErrors);
          ...
          <div className="order-2 lg:order-1">
            <div className="mb-8">
              <div className="flex items-center mb-4">
                ...
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold text-white leading-tight">FAN APPRECIATION PRIZE WHEEL</h1>
                  <p className="text-blue-300 font-semibold">PRESENTED BY CULVER'S</p>
                </div>
              </div>
              <h2 className="text-xl md:text-2xl font-bold text-white mb-2">SPIN TO WIN A PRIZE!</h2>
              <p className="text-gray-300 text-sm md:text-base">
                Rays Fans! Play the Rays Fan Appreciation Prize Wheel for your chance to instantly win exclusive Prizes from Culver's and your favorite ball club!
              </p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 bg-transparent border-b-2 text-white placeholder-gray-400 focus:outline-none focus:border-blue-400 transition-colors ${errors.firstName ? 'border-red-500' : 'border-gray-500'}`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 bg-transparent border-b-2 text-white placeholder-gray-400 focus:outline-none focus:border-blue-400 transition-colors ${errors.lastName ? 'border-red-500' : 'border-gray-500'}`}
                  />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 bg-transparent border-b-2 text-white placeholder-gray-400 focus:outline-none focus:border-blue-400 transition-colors ${errors.email ? 'border-red-500' : 'border-gray-500'}`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 bg-transparent border-b-2 text-white placeholder-gray-400 focus:outline-none focus:border-blue-400 transition-colors ${errors.phone ? 'border-red-500' : 'border-gray-500'}`}
              ...
              <div className="space-y-3 mt-6">
                <label className="flex items-start space-x-3 text-xs text-gray-300">
                  <input type="checkbox" checked={agreedToTerms} onChange={(e) => setAgreedToTerms(e.target.checked)} className="mt-1 w-4 h-4 text-blue-600 bg-transparent border-gray-400 rounded focus:ring-blue-500"/>
                  <span>I have read and agree to the <span className="text-blue-400 underline">Sweepstakes Rules</span> and the <span className="text-blue-400 underline">Terms of Use</span>.</span>
                </label>
                <label className="flex items-start space-x-3 text-xs text-gray-300">
                  <input type="checkbox" checked={agreedToEmails} onChange={(e) => setAgreedToEmails(e.target.checked)} className="mt-1 w-4 h-4 text-blue-600 bg-transparent border-gray-400 rounded focus:ring-blue-500"/>
                  <span>I would like to receive commercial e-mails from <span className="text-blue-400 underline">TampaBayRays.com</span>, <span className="text-blue-400 underline">MLB.com</span> and their partners.</span>
                </label>
                <label className="flex items-start space-x-3 text-xs text-gray-300">
                  <input type="checkbox" checked={agreedToExclusive} onChange={(e) => setAgreedToExclusive(e.target.checked)} className="mt-1 w-4 h-4 text-blue-600 bg-transparent border-gray-400 rounded focus:ring-blue-500"/>
                  <span>Subscribe to receive exclusive access to <span className="text-blue-400 underline">TheBayRepublic.com</span> product launches, sales, and more!</span>
                </label>
              </div>
              <button
                type="submit"
